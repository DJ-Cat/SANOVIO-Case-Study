 ...
 * something else somebody did mean to set — and they are exported apart, since
 * the page lays the picture and the description out in different columns.
 */
export function ProductPictures({ canonicalId, images }: {
  canonicalId: string | null; images: ProductImage[];
}) {
  const { t } = usePrefs();
  ...
  if (!canonicalId) {
    return (
      <div className="card grid aspect-square place-items-center p-6 text-center text-sm text-ink-400">
        {t("Confirm this row to give it a product page you can price and photograph.")}
      </div>
    );
  }
  return <Pictures canonicalId={canonicalId} images={images} />;
}

export function ProductDescription({ canonicalId, description }: {
  canonicalId: string; description: string | null;
}) {
  return <DescriptionPanel canonicalId={canonicalId} description={description} />;
}
...
function Pictures({ canonicalId, images }: { canonicalId: string; images: ProductImage[] }) {
  ...
  return (
    <div className="card space-y-3 p-4">
      <div className="grid aspect-square w-full place-items-center overflow-hidden rounded-2xl bg-white shadow-[var(--shadow-glow)] dark:bg-ink-800">
        {shot ? (
          /* eslint-disable-next-line @next/next/no-img-element */
          <img src={`/api/product-images/${shot.id}`} alt=""
            className="h-full w-full object-contain p-4" />
        ) : (
          <span className="text-xs text-ink-300">{t("No picture yet")}</span>
        )}
      </div>
          ...
          {images.map((img, i) => (
            <li key={img.id} className="flex items-center gap-2 text-[11px] text-ink-400">
              ...
              <img src={`/api/product-images/${img.id}`} alt=""
                className="h-8 w-8 shrink-0 rounded-lg border hair bg-white object-contain p-0.5 dark:border-ink-700 dark:bg-ink-800" />
              <span className="min-w-0 flex-1 truncate">
                {i === 0 && <span className="font-medium text-ink-500 dark:text-ink-300">{t("shown")} · </span>}
                {img.confidence === "certain"
                  ? t("supplied by you")
                  : `${img.page ? t("from your catalogue, p.{page}", { page: img.page }) : t("from your catalogue")} · ${t(img.confidence)}`}
              </span>
              <form action={removeProductImage.bind(null, img.id)} className="shrink-0">
                <button title={t("Remove this picture")}
                  className="rounded-lg p-1 text-ink-400 transition hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/10">
      ...
      <form action={action} className="space-y-2 border-t border-ink-50 pt-3 dark:border-ink-800">
        ...
        <label className="block cursor-pointer rounded-xl border border-dashed border-ink-100 px-3 py-2.5 text-center text-xs transition hover:border-brand-300 hover:bg-ink-25/60 dark:border-ink-700 dark:hover:border-brand-500/50">
          ...
          <span className="font-medium text-ink-600 dark:text-ink-200">
            {chosen ?? t("Add your own photograph")}
          </span>
        </label>
  ...
  return (
    <form action={action} className="card flex flex-col gap-2 p-5">
      ...
      <textarea name="description" rows={5} defaultValue={description ?? ""} disabled={pending}
        placeholder={t("What this article is for, and anything a buyer should know before switching to it.")}
        className="min-h-32 w-full flex-1 resize-y rounded-lg border hair-strong bg-[var(--sheet)] outline-none transition focus:border-brand-300 focus:ring-4 focus:ring-brand-500/10 px-3 py-2 text-sm disabled:opacity-50" />
      <p className="text-[11px] text-ink-400">
        {t("Shown to hospitals on the product page. Extraction never fills this in — a catalogue describes a family, not a size.")}
      </p>
      <div className="flex flex-wrap items-center gap-3">
        <button type="submit" disabled={pending}
          className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-white font-semibold text-ink-700 shadow-[0_0_0_1px_var(--line-strong),0_1px_2px_rgb(20_21_40/0.04)] transition-all hover:text-brand-700 hover:shadow-[0_0_0_1px_var(--color-brand-200),0_4px_14px_-4px_rgb(87_89_242/0.30)] disabled:opacity-40 dark:bg-ink-900 dark:text-ink-100 px-3 py-1.5 text-xs">
          {pending ? t("Saving…") : t("Save description")}
        </button>
        <Banner state={state} />
      </div>
    </form>
