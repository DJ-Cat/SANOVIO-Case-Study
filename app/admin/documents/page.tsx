...
export default function Documents() {
  ...
  return (
    <Page title="Uploaded documents"
      lead="Every uploaded file is stored verbatim. Extraction is re-runnable against the original, and a savings claim can always be traced back to the document it came from.">
      {docs.length === 0 ? <Empty>No documents uploaded.</Empty> : (
        <Table head={["File", "Organisation", "Kind", "Rows", "Size", "Uploaded", "Original"]}>
          {docs.map((d) => (
            <tr key={d.id}>
